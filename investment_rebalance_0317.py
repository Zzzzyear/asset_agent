...
def generate_report(data: pd.DataFrame, strategy_nav: pd.Series) -> None:
    ...
    # 新增：添加gold:bond:strategy说明
    ratio_text = (
        f"Allocation - Gold: {STRATEGY_CONFIG['assets']['gold']['weight'] * 100:.0f}%, "
        f"Bond: {STRATEGY_CONFIG['assets']['bond']['weight'] * 100:.0f}%, "
        f"Strategy: {STRATEGY_CONFIG['assets']['strategy']['weight'] * 100:.0f}%"
    )
    # 在图的下方中间添加文本说明
    plt.figtext(0.5, 0.01, ratio_text, wrap=True, horizontalalignment='center', fontsize=10)
...
def run_re_average(gold: str, bond: str, start_date: str = "2022-01-01", rebalance_ratio: str = "25:25:50") -> Dict:
    # 根据传入参数更新全局策略配置
